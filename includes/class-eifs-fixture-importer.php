...
if ( ! class_exists( 'EIFS_Fixture_Importer' ) ) {
	class EIFS_Fixture_Importer extends SP_Importer {
		 ...
		 * @return void
		 */
		function import( $array = array(), $columns = array( 'post_title' ) ): void {
			...
			// Get event format, league, and season from post vars with proper sanitization
			$event_format = ( empty( $_POST['sp_format'] ) ? false : sanitize_key( $_POST['sp_format'] ) );
			$league       = ( sp_array_value( $_POST, 'sp_league', '-1' ) == '-1' ? false : sanitize_text_field( wp_unslash( $_POST['sp_league'] ) ) );
			$season       = ( sp_array_value( $_POST, 'sp_season', '-1' ) == '-1' ? false : sanitize_text_field( wp_unslash( $_POST['sp_season'] ) ) );
